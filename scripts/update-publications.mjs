...
// every DOI already in js/publications-data.js or in
// scripts/publications-ignore.json, and inserts ready-made entries below
// the AUTO-INSERT marker. The workflow then opens a pull request so a lab
...
function toEntry(w) {
    ...
    const L = [];
    L.push(`    // @${isArticle ? 'article' : 'inproceedings'}{${key}, — added automatically from OpenAlex, please verify`);
    L.push('    {');
    L.push(`        type: '${isArticle ? 'article' : 'inproceedings'}',`);
    L.push(`        key: '${key}',`);
    L.push(`        title: '${escJs(w.title)}',`);
    L.push(`        author: [${authors.map(a => `'${escJs(a)}'`).join(', ')}],`);
    if (venue) L.push(`        ${isArticle ? 'journal' : 'booktitle'}: '${escJs(venue)}',`);
    if (b.volume) L.push(`        volume: '${escJs(b.volume)}',`);
    if (b.issue) L.push(`        number: '${escJs(b.issue)}',`);
    if (pages) L.push(`        pages: '${escJs(pages)}',`);
    L.push(`        doi: '${escJs(doi)}',`);
    L.push(`        year: ${year},`);
    L.push(`        scholar: '',`);
    L.push('    },');
    return L.join('\n');
...
async function main() {
    ...
    if (!dataSrc.includes(MARKER)) throw new Error(`Marker "${MARKER}" not found in ${DATA_FILE}`);

    const existing = new Set([...dataSrc.matchAll(/doi:\s*'([^']+)'/g)].map(m => normDoi(m[1])));
    const ignored = new Set(JSON.parse(readFileSync(IGNORE_FILE, 'utf8')).map(normDoi));

    const works = await fetchAllWorks();
    const fresh = works.filter(w =>
        w.doi && w.title && w.publication_year >= 2021 &&
        !SKIP_TYPES.has(w.type) &&
        !existing.has(normDoi(w.doi)) &&
        !ignored.has(normDoi(w.doi)));

    // Dedupe within this batch (OpenAlex occasionally lists a work twice)
    const seen = new Set();
    const unique = fresh.filter(w => {
    ...
    const markerLine = dataSrc.split('\n').find(l => l.includes(MARKER));
    const block = markerLine + '\n\n' + unique.map(toEntry).join('\n\n') + '\n';
    writeFileSync(DATA_FILE, dataSrc.replace(markerLine, block));

    const body = [
        `This automated weekly check found **${unique.length}** publication(s) on ` +
    ...
    console.log(`Inserted ${unique.length} new publication(s) into ${DATA_FILE}:`);
    unique.forEach(w => console.log(`  - ${w.title} (${normDoi(w.doi)})`));
}
